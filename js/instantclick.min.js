var InstantClick=function(document,location){var $ua=navigator.userAgent,$isChromeForIOS=$ua.indexOf(' CriOS/')>-1,$hasTouch='createTouch'in document,$currentLocationWithoutHash,$urlToPreload,$preloadTimer,$lastTouchTimestamp,$history={},$xhr,$url=false,$title=false,$mustRedirect=false,$body=false,$head=false,$timing={},$isPreloading=false,$isWaitingForCompletion=false,$trackedAssets=[],$useWhitelist,$preloadOnMousedown,$delayBeforePreload,$eventsCallbacks={fetch:[],receive:[],wait:[],change:[]}
...
return returnValue}
function changePage(title,body,headCache,newUrl,scrollY){document.documentElement.replaceChild(body,document.body)
...
function loopHeadTags(head,callback){for(const i of head.children){let v
switch(i.tagName){case 'LINK':const rel=i.getAttribute('rel')||''
if(rel&&!stylesheetRegex.test(rel)){v=rel}
break;case 'META':const name=i.getAttribute('name')||''
const property=i.getAttribute('property')||''
if(name||property){v=`${name}-${property}`}
break;}
if(v){callback(i,`${i.tagName}-${v}`)}}}
function makeHeadCache(head){const cache={}
loopHeadTags(head,(i,key)=>{cache[key]=i.outerHTML})
return cache}
function updateHead(headCache){loopHeadTags(document.head,(i,key)=>{const headTag=headCache[key]
if(headTag){i.outerHTML=headTag}})}
...
$history[urlWithoutHash]={body:$body,title:$title,head:$head,scrollY:urlWithoutHash in $history?$history[urlWithoutHash].scrollY:0}
...
$history[$currentLocationWithoutHash]={body:document.body,title:document.title,head:makeHeadCache(document.head),scrollY:pageYOffset}
